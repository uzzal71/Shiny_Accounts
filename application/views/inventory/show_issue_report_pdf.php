...
$_POST['recieve_issue_report_info'] = $recieve_issue_report_info;
...
// Page header
    function Header()
    {
        ...
        $this->SetFont('Courier','B',14);
        $this->Cell(80);
        $this->Cell(30,0,'Issue Report ',0,0,'C');
        // Line break
        $this->Ln(5);
//Water Mark Start
...
// Colored table
    function FancyTable($header, $data)
    {
        ...
        // $totalGovtHoliday = 0;
        $sl = 0;
        $recieve_issue_report_info = $_POST['recieve_issue_report_info'];

        //  print '<pre>';
        // print_r($recieve_issue_report_info);
        // die();

        if($recieve_issue_report_info){
        foreach ($recieve_issue_report_info as  $each_issue){
    
            $this->Cell(1);
            $this->Cell($w[0],6,$each_issue->issue_id,'LR',0,'C',$fill);
            $log_data =json_decode ($each_issue->issue_info);
            $this->Ln();
...
$pdf->SetFont('Times','',8);
$pdf->AddPage();
$pdf->FancyTable($header,$recieve_issue_report_info);
$pdf->Output();
?>
