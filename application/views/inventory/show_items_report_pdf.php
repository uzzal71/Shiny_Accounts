...
$_POST['items_report_info'] = $items_report_info;
...
// Page header
    function Header()
    {
        ...
        $this->SetFont('Courier','B',14);
        $this->Cell(80);
        $this->Cell(30,0,'Items Report',0,0,'C');
        // Line break
        $this->Ln(5);
//Water Mark Start
...
// Colored table
    function FancyTable($header, $data)
    {
        ...
        // $totalGovtHoliday = 0;
        $sl = 0;
        $items_report_info = $_POST['items_report_info'];
        if($items_report_info){
        foreach($items_report_info as $row)
        {
            $sl++;
   //          $date = $row->from_date;
   ...
   //          }
            ...
            $this->Cell($w[1],6,$row->item_id,'LR',0,'C',$fill);
            $this->Cell($w[2],6,$row->item_name,'LR',0,'C',$fill);
            $this->Cell($w[3],6,$row->quantity,'LR',0,'C',$fill);
            $this->Cell($w[4],6,$row->unit_price,'LR',0,'C',$fill);
            $this->Cell($w[5],6,$row->remarks,'LR',0,'C',$fill);
            $this->Ln();
            $fill = !$fill;
        }
  //   }
...
$pdf = new PDF();
// Column headings
$header = array('SL','Item ID','Item Name', 'Quantity','Unit Price','DESCRIPTION');
// Data loading
//$data = $pdf->LoadData('countries.txt');
$pdf->SetFont('Times','',8);
$pdf->AddPage();
$pdf->FancyTable($header,$items_report_info);
$pdf->Output();
?>
