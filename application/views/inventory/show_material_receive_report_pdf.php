...
$_POST['recieve_items_report_info'] = $recieve_items_report_info;
...
// Page header
    function Header()
    {
        ...
        $this->SetFont('Courier','B',14);
        $this->Cell(80);
        $this->Cell(30,0,'Material Recieve Report ',0,0,'C');
        // Line break
        $this->Ln(5);
//Water Mark Start
...
// Better table
    function ImprovedTable($header, $data)
    {
        ...
        foreach($data as $row)
        {
            ...
            $this->Cell($w[2],7,number_format($row[2]),'LR',0,'R');
            $this->Cell($w[3],7,number_format($row[3]),'LR',0,'R');
...
// Colored table
    function FancyTable($header, $data)
    {
        ...
        // $totalGovtHoliday = 0;
        $sl = 0;
        $recieve_items_report_info = $_POST['recieve_items_report_info'];

            // print '<pre>';
        // print_r($recieve_items_report_info);
        // die();

        if($recieve_items_report_info){
        foreach ($recieve_items_report_info as  $each_recieved_info){
    
            $this->Cell(1);
            $this->Cell($w[0],7,$each_recieved_info->log_id,'LR',0,'C',$fill);
            ...
            foreach ($log_data as $key => $log_info) {
                ...
                $this->Cell(1);
                $this->Cell($w[1],7,$log_info->item_name,0,'C',$fill);
                $this->Cell($w[2],7,$log_info->item_id,'LR',0,'C',$fill);
                $this->Cell($w[3],7,$log_info->quantity,'LR',0,'C',$fill);
                $this->Cell($w[4],7,$log_info->unit_price,'LR',0,'C',$fill);
                $this->Cell($w[5],7,$log_info->total_price,'LR',0,'C',$fill);
                $this->Cell($w[6],7,$log_info->supplier_id,'LR',0,'C',$fill);
                $this->Ln();
                // $fill = !$fill;
             }
   }
...
$pdf = new PDF();
// Column headings
$header = array('Recieve Id','Item Name','Item ID', 'Quantity','Unit Price','Total Price','Supplier ID');
// Data loading
//$data = $pdf->LoadData('countries.txt');
$pdf->SetFont('Times','',8);
$pdf->AddPage();
$pdf->FancyTable($header,$recieve_items_report_info);
$pdf->Output();
?>
